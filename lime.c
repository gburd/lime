...
#define LIME_VERSION_STRING "0.2.3"
#endif
...
/*
** Generate a *_snapshot.c file that contains the create_base_snapshot()
** implementation.  This function allocates a ParserSnapshot struct and
** populates it with the action tables computed by ReportTable().
**
** The generated file is self-contained: it includes snapshot.h and
** uses malloc/memcpy to build the snapshot at runtime.
*/
void ReportSnapshotInit(struct lime *lemp)
{
  ...
  fprintf(out,
    "/*\n"
    "** Snapshot initialization code generated by Lemon from \"%s\".\n"
  ...
  /* Emit a static helper to get wall-clock nanoseconds */
  fprintf(out,
  ...
  /* Begin the create_base_snapshot function */
  fprintf(out,
    "ParserSnapshot *create_base_snapshot(\n"
    ...
    "        if (error) {\n"
    "            *error = malloc(32);\n"
    "            if (*error) memcpy(*error, \"allocation failed\", 18);\n"
    "        }\n"
    "        return NULL;\n"
    "    }\n"
    "\n"
    ...
    "    snap->create_time_ns = get_time_ns();\n"
    "    snap->jit_ctx = NULL;\n"
    "\n"
  );
  ...
  /* Emit the yy_action array */
  n = lemp->nactiontab;
  fprintf(out,
    "    /* Action table: %d entries */\n"
    "    snap->action_count = %d;\n"
    "    snap->yy_action = (uint16_t *)malloc(%d * sizeof(uint16_t));\n"
    "    if (snap->yy_action == NULL) goto fail;\n"
    "    {\n"
    "        static const uint16_t action_data[] = {\n",
    n, n, n
  );
  for(i=j=0; i<n; i++){
  ...
  fprintf(out,
    "        };\n"
    "        memcpy(snap->yy_action, action_data, %d * sizeof(uint16_t));\n"
    "    }\n\n",
    n
  );

  /* Emit the yy_lookahead array (with padding) */
  nLookAhead = lemp->nLookahead;
  fprintf(out,
    "    /* Lookahead table: %d entries */\n"
    "    snap->lookahead_count = %d;\n"
    "    snap->yy_lookahead = (uint16_t *)malloc(%d * sizeof(uint16_t));\n"
    "    if (snap->yy_lookahead == NULL) goto fail;\n"
    "    {\n"
    "        static const uint16_t lookahead_data[] = {\n",
    nLookAhead, nLookAhead, nLookAhead
  );
  for(i=j=0; i<nLookAhead; i++){
    int la = lemp->aLookahead ? lemp->aLookahead[i] : 0;
    if( j==0 ) fprintf(out,"            ");
    fprintf(out, "%d,", la);
    if( j==9 || i==nLookAhead-1 ){
      fprintf(out, "\n");
      j = 0;
    }else{
      j++;
  ...
  fprintf(out,
    "        };\n"
    "        memcpy(snap->yy_lookahead, lookahead_data, %d * sizeof(uint16_t));\n"
    "    }\n\n",
    nLookAhead
  );

  /* Emit the yy_shift_ofst array */
  n = lemp->nxstate;
  fprintf(out,
    "    /* Shift offset table: %d entries */\n"
    "    snap->yy_shift_ofst = (int16_t *)malloc(%d * sizeof(int16_t));\n"
    "    if (snap->yy_shift_ofst == NULL) goto fail;\n"
    "    {\n"
    "        static const int16_t shift_ofst_data[] = {\n",
    n, n
  );
  for(i=j=0; i<n; i++){
    int ofst;
  ...
  fprintf(out,
    "        };\n"
    "        memcpy(snap->yy_shift_ofst, shift_ofst_data, %d * sizeof(int16_t));\n"
    "    }\n\n",
    n
  );

  /* Emit the yy_reduce_ofst array */
  {
    int mnNtOfst = 0;
    for(i=0; i<lemp->nxstate; i++){
      stp = lemp->sorted[i];
    ...
    fprintf(out,
      "    /* Reduce offset table: %d entries */\n"
      "    snap->yy_reduce_ofst = (int16_t *)malloc(%d * sizeof(int16_t));\n"
      "    if (snap->yy_reduce_ofst == NULL) goto fail;\n"
      "    {\n"
      "        static const int16_t reduce_ofst_data[] = {\n",
      n, n
    );
    for(i=j=0; i<n; i++){
      int ofst;
    ...
    fprintf(out,
      "        };\n"
      "        memcpy(snap->yy_reduce_ofst, reduce_ofst_data, %d * sizeof(int16_t));\n"
      "    }\n\n",
      n
    );
  }

  /* Emit the yy_default array */
  fprintf(out,
    "    /* Default action table: %d entries */\n"
    "    snap->yy_default = (uint16_t *)malloc(%d * sizeof(uint16_t));\n"
    "    if (snap->yy_default == NULL) goto fail;\n"
    "    {\n"
    "        static const uint16_t default_data[] = {\n",
    n, n
  );
  for(i=j=0; i<n; i++){
    stp = lemp->sorted[i];
  ...
  fprintf(out,
    "        };\n"
    "        memcpy(snap->yy_default, default_data, %d * sizeof(uint16_t));\n"
    "    }\n\n",
    n
  );

  /* Symbols, rules, and states pointers are left NULL for now.
  ** They will be populated when deep-copy support is added. */
  fprintf(out,
    "    /* Symbol/rule/state deep copies not yet implemented */\n"
    "    snap->symbols = NULL;\n"
    "    snap->rules = NULL;\n"
    "    snap->states = NULL;\n"
    "\n"
