...
**
** Threshold: distance <= max(1, len/3), capped at 3.  Scaling lets
** short names require an exact match (3-letter `OR` won't suggest
** `AND`) while longer names tolerate one or two typos.  Matches
** rustc's heuristic. */
...
static int lint_grammar(struct lime *lem){
  ...
  if( lem->exports ){
    for(struct exported_symbol *exp = lem->exports; exp; exp = exp->next){
      ...
      if( !sp ){
        /* Suggest the closest defined symbol.  Exports normally name
        ** non-terminals, so that's the preferred kind; pass 2 of
        ** lint_suggest_similar widens to terminals if no NT is close. */
