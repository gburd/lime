...
/*
** lime_lex_regex_free -- post-order free of a regex AST.
**
** ----------------------------------------------------------------------
** TAIL-CALL STRUCTURE -- LOAD-BEARING, see src/merkle_tree.c
** ----------------------------------------------------------------------
**
** Right-recurse via the outer while loop so a deep right-spine of
** concatenations or alternations does not blow the stack.  Pathological
** inputs like `(((...(a|b)...)))` can reach AST depth in the thousands
** when authoring a generated lexer; without this loop a single
** lime_lex_regex_free() call could overflow the default 8 MB stack on
** Linux or 512 KB stack on Darwin.  The left subtree is still freed via
** a regular recursive call (its depth is bounded by the matching number
** of recursive parses, which the parser already rejects past
** LIME_LEX_REGEX_MAX_DEPTH), so this is the right structural fix.
*/
void lime_lex_regex_free(LimeReNode *n) {
    while (n != NULL) {
        LimeReNode *next = NULL;
        switch (n->kind) {
        case LIME_RE_CONCAT:
        case LIME_RE_ALT:
            lime_lex_regex_free(n->u.binary.left);  /* left: ordinary recursion */
            next = n->u.binary.right;               /* right: loop -> TCO */
            break;
        case LIME_RE_STAR:
        case LIME_RE_PLUS:
        case LIME_RE_QUESTION:
            next = n->u.unary.child;
            break;
        case LIME_RE_REPEAT:
            next = n->u.repeat.child;
            break;
        default:
            break;
        }
    ...
    ** ambiguous. */
    ...
    while (!at_end(p) && (first || peek(p) != ']')) {
        ...
        if (peek(p) == '-' && peek_at(p, 1) != ']' && peek_at(p, 1) != -1) {
            ...
            if (byte > hi) {
                set_error(p, "character class range out of order: %d-%d", byte, hi);
                lime_lex_regex_free(n);
                return NULL;
            }
            for (int b = byte; b <= hi; b++) {
                lime_lex_regex_class_set(n->u.char_class.bits, (unsigned char)b);
            }
        } else {
            lime_lex_regex_class_set(n->u.char_class.bits, (unsigned char)byte);
        }
    }
    if (peek(p) != ']') {
...
LimeReNode *lime_lex_regex_parse(const char *src, char **err_out) {
    ...
    if (!at_end(&p)) {
        set_error(&p, "unexpected '%c' at column %ld", *p.p, (long)(p.p - p.src));
        lime_lex_regex_free(root);
        return NULL;
    }
    if (!root) {
