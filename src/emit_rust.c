...
static void emit_parser_runtime(FILE *out, const struct lime *lemp) {
    ...
    ** in a method on the parser; otherwise emit a no-op default. */
    ...
    fprintf(out,
            ...
            "    }\n\n"
            "    /// Current LALR(1) state at the top of the parse stack.\n"
            "    /// Public so wrappers (e.g. the lalrpop skin) can call\n"
            "    /// `expected_tokens_in_state(parser.current_state())` to\n"
            "    /// build the set of tokens that would be accepted next.\n"
            "    /// Stable: part of the documented introspection surface.\n"
            "    #[inline]\n"
