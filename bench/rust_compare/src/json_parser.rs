// SPDX-License-Identifier: <inherited from input grammar>
//
// Generated by lime -- DO NOT EDIT BY HAND.
//
// Source grammar: bench/rust_compare/json.lime
// Parser name:    JsonParse
// To regenerate:  lime --rust bench/rust_compare/json.lime
//

#![allow(dead_code)]
#![allow(unused_assignments)]
#![allow(unused_variables)]
#![allow(unused_mut)]
#![allow(unused_parens)]
#![allow(non_snake_case)]
#![allow(non_upper_case_globals)]
#![allow(clippy::all)]

// ======================================================================
// Token codes (external = internal + FIRST_TOKEN)
// ======================================================================

pub const FIRST_TOKEN: u16 = 257;

pub const LBRACE: u16 = 258;
pub const RBRACE: u16 = 259;
pub const LBRACKET: u16 = 260;
pub const RBRACKET: u16 = 261;
pub const COLON: u16 = 262;
pub const COMMA: u16 = 263;
pub const STRING: u16 = 264;
pub const NUMBER: u16 = 265;
pub const TRUE: u16 = 266;
pub const FALSE: u16 = 267;
pub const NULL: u16 = 268;


// ======================================================================
// Dispatch range constants
// ======================================================================

pub const NSTATE:    u32 = 10;
pub const NRULE:     u32 = 17;
pub const NTERMINAL: u32 = 12;
pub const NSYMBOL:   u32 = 19;
pub const NTOKEN:    u16 = 12;
pub const YY_MAX_SHIFT:        u16 = 9;
pub const YY_MIN_SHIFTREDUCE:  u16 = 26;
pub const YY_MAX_SHIFTREDUCE:  u16 = 42;
pub const YY_ERROR_ACTION:     u16 = 43;
pub const YY_ACCEPT_ACTION:    u16 = 44;
pub const YY_NO_ACTION:        u16 = 45;
pub const YY_MIN_REDUCE:       u16 = 46;


// ======================================================================
// Action tables
// ======================================================================

pub static YY_ACTION: &[u16] = &[
       57,     4,    45,     1,    39,     7,     7,    29,    30,    31,
       32,    33,     4,     8,     1,    62,    62,    62,    29,    30,
       31,    32,    33,     3,    44,     9,     9,     9,     6,     6,
        6,    46,    34,     6,    58,    58,    58,     8,    40,    35,
        2,    45,    45,     5,
];
pub static YY_LOOKAHEAD: &[u16] = &[
       17,     1,    19,     3,     4,    16,    17,     7,     8,     9,
       10,    11,     1,     7,     3,    13,    14,    15,     7,     8,
        9,    10,    11,     5,    12,    13,    14,    15,    13,    14,
       15,     0,     2,    18,    13,    14,    15,     7,     4,     2,
        6,    19,    19,     6,    19,    19,    19,    19,    19,    19,
       19,    19,    19,    19,    19,    19,
];
pub static YY_SHIFT_OFST: &[i32] = &[
        11,      0,     11,     11,     30,      6,     34,     37,     18,     31,
   
];
pub static YY_REDUCE_OFST: &[i32] = &[
        12,     15,      2,     21,    -11,    -17, -2147483647, -2147483647, -2147483647, -2147483647,
   
];
pub static YY_DEFAULT: &[u16] = &[
       43,    43,    43,    43,    43,    43,    43,    43,    43,    43,
   
];
pub static YY_RULE_LHS: &[i16] = &[
       12,    13,    13,    13,    13,    13,    13,    13,    14,    14,    16,    16,
       17,    15,    15,    18,    18,
];
pub static YY_RULE_NRHS: &[i8] = &[
      -1,   -1,   -1,   -1,   -1,   -1,   -1,   -1,   -2,   -3,   -1,   -3,   -3,   -2,   -3,   -1,
      -3,
];

pub const HAS_FALLBACK: bool = false;
pub static YY_FALLBACK: &[u16] = &[];


// ======================================================================
// Per-rule reduce callbacks
// ======================================================================

pub type UserArg = ();

/// Context passed to each rule's reduce callback.
/// Rules read RHS values via the `rhs` slice, write LHS via
/// `lhs`, and access the user arg (if any) via `user`.
pub struct ReduceCtx<'a> {
    pub lhs:  &'a mut Value,
    pub rhs:  &'a mut [Value],
    pub user: &'a mut UserArg,
}

/// Default semantic Value type.  Override via
/// %rust_value_type {T} in the grammar to use a
/// custom type (String, struct, Box<dyn Any>, etc.).
pub type Value = i64;

/// Rule 0: nrhs=1, lhs symbol index=12 (line 0)
fn yy_rule_0(ctx: &mut ReduceCtx) {
    let mut lhs: Value = Value::default();
    let rhs0: Value = ctx.rhs[0].clone();
    lhs = rhs0;  // default: $$ = $1
    *ctx.lhs = lhs;
}

/// Rule 1: nrhs=1, lhs symbol index=13 (line 0)
fn yy_rule_1(ctx: &mut ReduceCtx) {
    let mut lhs: Value = Value::default();
    let rhs0: Value = ctx.rhs[0].clone();
    lhs = rhs0;  // default: $$ = $1
    *ctx.lhs = lhs;
}

/// Rule 2: nrhs=1, lhs symbol index=13 (line 0)
fn yy_rule_2(ctx: &mut ReduceCtx) {
    let mut lhs: Value = Value::default();
    let rhs0: Value = ctx.rhs[0].clone();
    lhs = rhs0;  // default: $$ = $1
    *ctx.lhs = lhs;
}

/// Rule 3: nrhs=1, lhs symbol index=13 (line 0)
fn yy_rule_3(ctx: &mut ReduceCtx) {
    let mut lhs: Value = Value::default();
    let rhs0: Value = ctx.rhs[0].clone();
    lhs = rhs0;  // default: $$ = $1
    *ctx.lhs = lhs;
}

/// Rule 4: nrhs=1, lhs symbol index=13 (line 0)
fn yy_rule_4(ctx: &mut ReduceCtx) {
    let mut lhs: Value = Value::default();
    let rhs0: Value = ctx.rhs[0].clone();
    lhs = rhs0;  // default: $$ = $1
    *ctx.lhs = lhs;
}

/// Rule 5: nrhs=1, lhs symbol index=13 (line 0)
fn yy_rule_5(ctx: &mut ReduceCtx) {
    let mut lhs: Value = Value::default();
    let rhs0: Value = ctx.rhs[0].clone();
    lhs = rhs0;  // default: $$ = $1
    *ctx.lhs = lhs;
}

/// Rule 6: nrhs=1, lhs symbol index=13 (line 0)
fn yy_rule_6(ctx: &mut ReduceCtx) {
    let mut lhs: Value = Value::default();
    let rhs0: Value = ctx.rhs[0].clone();
    lhs = rhs0;  // default: $$ = $1
    *ctx.lhs = lhs;
}

/// Rule 7: nrhs=1, lhs symbol index=13 (line 0)
fn yy_rule_7(ctx: &mut ReduceCtx) {
    let mut lhs: Value = Value::default();
    let rhs0: Value = ctx.rhs[0].clone();
    lhs = rhs0;  // default: $$ = $1
    *ctx.lhs = lhs;
}

/// Rule 8: nrhs=2, lhs symbol index=14 (line 0)
fn yy_rule_8(ctx: &mut ReduceCtx) {
    let mut lhs: Value = Value::default();
    let rhs0: Value = ctx.rhs[0].clone();
    let _rhs1: Value = ctx.rhs[1].clone();
    lhs = rhs0;  // default: $$ = $1
    *ctx.lhs = lhs;
}

/// Rule 9: nrhs=3, lhs symbol index=14 (line 0)
fn yy_rule_9(ctx: &mut ReduceCtx) {
    let mut lhs: Value = Value::default();
    let rhs0: Value = ctx.rhs[0].clone();
    let _rhs1: Value = ctx.rhs[1].clone();
    let _rhs2: Value = ctx.rhs[2].clone();
    lhs = rhs0;  // default: $$ = $1
    *ctx.lhs = lhs;
}

/// Rule 10: nrhs=1, lhs symbol index=16 (line 0)
fn yy_rule_10(ctx: &mut ReduceCtx) {
    let mut lhs: Value = Value::default();
    let rhs0: Value = ctx.rhs[0].clone();
    lhs = rhs0;  // default: $$ = $1
    *ctx.lhs = lhs;
}

/// Rule 11: nrhs=3, lhs symbol index=16 (line 0)
fn yy_rule_11(ctx: &mut ReduceCtx) {
    let mut lhs: Value = Value::default();
    let rhs0: Value = ctx.rhs[0].clone();
    let _rhs1: Value = ctx.rhs[1].clone();
    let _rhs2: Value = ctx.rhs[2].clone();
    lhs = rhs0;  // default: $$ = $1
    *ctx.lhs = lhs;
}

/// Rule 12: nrhs=3, lhs symbol index=17 (line 0)
fn yy_rule_12(ctx: &mut ReduceCtx) {
    let mut lhs: Value = Value::default();
    let rhs0: Value = ctx.rhs[0].clone();
    let _rhs1: Value = ctx.rhs[1].clone();
    let _rhs2: Value = ctx.rhs[2].clone();
    lhs = rhs0;  // default: $$ = $1
    *ctx.lhs = lhs;
}

/// Rule 13: nrhs=2, lhs symbol index=15 (line 0)
fn yy_rule_13(ctx: &mut ReduceCtx) {
    let mut lhs: Value = Value::default();
    let rhs0: Value = ctx.rhs[0].clone();
    let _rhs1: Value = ctx.rhs[1].clone();
    lhs = rhs0;  // default: $$ = $1
    *ctx.lhs = lhs;
}

/// Rule 14: nrhs=3, lhs symbol index=15 (line 0)
fn yy_rule_14(ctx: &mut ReduceCtx) {
    let mut lhs: Value = Value::default();
    let rhs0: Value = ctx.rhs[0].clone();
    let _rhs1: Value = ctx.rhs[1].clone();
    let _rhs2: Value = ctx.rhs[2].clone();
    lhs = rhs0;  // default: $$ = $1
    *ctx.lhs = lhs;
}

/// Rule 15: nrhs=1, lhs symbol index=18 (line 0)
fn yy_rule_15(ctx: &mut ReduceCtx) {
    let mut lhs: Value = Value::default();
    let rhs0: Value = ctx.rhs[0].clone();
    lhs = rhs0;  // default: $$ = $1
    *ctx.lhs = lhs;
}

/// Rule 16: nrhs=3, lhs symbol index=18 (line 0)
fn yy_rule_16(ctx: &mut ReduceCtx) {
    let mut lhs: Value = Value::default();
    let rhs0: Value = ctx.rhs[0].clone();
    let _rhs1: Value = ctx.rhs[1].clone();
    let _rhs2: Value = ctx.rhs[2].clone();
    lhs = rhs0;  // default: $$ = $1
    *ctx.lhs = lhs;
}

/// Dispatch table: ruleno -> reduce-callback function.
pub static YY_RULE_REDUCE_FN: [fn(&mut ReduceCtx); NRULE as usize] = [
    yy_rule_0,
    yy_rule_1,
    yy_rule_2,
    yy_rule_3,
    yy_rule_4,
    yy_rule_5,
    yy_rule_6,
    yy_rule_7,
    yy_rule_8,
    yy_rule_9,
    yy_rule_10,
    yy_rule_11,
    yy_rule_12,
    yy_rule_13,
    yy_rule_14,
    yy_rule_15,
    yy_rule_16,
];

// ======================================================================
// Parser API + LALR runtime
// ======================================================================

#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ParseError {
    /// Token had no shift action at the current state.
    SyntaxError { token: u16, state: u16 },
    /// External token was outside [FIRST_TOKEN, FIRST_TOKEN+NTOKEN).
    OutOfRange { token: u16 },
    /// Reduce action attempted on rule index >= NRULE.
    BadReduce { rule: u32 },
    /// Stack underflow during reduce (table corruption).
    StackUnderflow,
}

#[derive(Debug, Clone, Default)]
struct Frame {
    state: u16,
    major: u16,
    value: Value,
}

/// JsonParse parser.
pub struct JsonParseParser {
    stack: Vec<Frame>,
    accepted: bool,
    errored: bool,
    pub final_value: Value,
    /// User argument threaded into ReduceCtx for
    /// every reduce callback.  Populated from
    /// %rust_extra_argument; () when unset.
    pub user: UserArg,
    /// Scratch buffer for rhs values, reused across
    /// reduces.  Avoids per-reduce Vec allocation
    /// in the LALR loop hot path.
    rhs_scratch: Vec<Value>,
}

impl JsonParseParser {
    /// Construct a parser with a user-supplied UserArg.
    /// When UserArg is () use new() instead.
    pub fn new_with_user(user: UserArg) -> Self {
        let mut stack = Vec::with_capacity(64);
        stack.push(Frame::default());
        Self {
            stack, accepted: false, errored: false,
            final_value: Value::default(),
            user,
            rhs_scratch: Vec::with_capacity(16),
        }
    }

    /// Construct a fresh parser at state 0 with the
    /// UserArg's Default value.
    pub fn new() -> Self where UserArg: Default {
        Self::new_with_user(UserArg::default())
    }

    /// Feed an external token code (with semantic value).
    /// Use 0 as the token code to indicate end-of-input.
    /// Returns Ok on accept-pending or accept; Err on syntax
    /// error or unrecoverable state corruption.
    pub fn push(&mut self, token_code: u16, value: Value)
        -> Result<bool, ParseError>
    {
        if self.accepted { return Ok(true); }
        if self.errored  { return Err(ParseError::SyntaxError {
            token: token_code,
            state: self.top_state(),
        }); }

        // Convert external -> internal: external == internal + FIRST_TOKEN.
        // EOF (0) is preserved; out-of-range yields OutOfRange.
        let mut major: i32 = if token_code == 0 {
            0
        } else {
            (token_code as i32) - (FIRST_TOKEN as i32)
        };
        if major < 0 || major >= (NTOKEN as i32) {
            self.errored = true;
            return Err(ParseError::OutOfRange { token: token_code });
        }

        let mut retried_with_fallback = false;
        loop {
            // Top state.
            let cur_state = self.top_state();
            // Pending shift-reduce: cur_state encoded as a reduce.
            if cur_state >= YY_MIN_REDUCE {
                let ruleno = (cur_state - YY_MIN_REDUCE) as u32;
                match self.reduce(ruleno)? {
                    ReduceOutcome::Continue => continue,
                    ReduceOutcome::Accept => {
                        self.accepted = true;
                        return Ok(true);
                    }
                }
            }

            let action = Self::find_shift_action(cur_state, major as u16);

            // Plain shift: push (action, major) and consume.
            if action <= YY_MAX_SHIFT {
                if token_code == 0 {
                    self.accepted = true;
                    self.on_parse_accept();
                    return Ok(true);
                }
                self.stack.push(Frame {
                    state: action,
                    major: major as u16,
                    value,
                });
                return Ok(false);
            }

            // Shift-reduce: encode the action as a pending reduce
            // (action + MIN_REDUCE - MIN_SHIFTREDUCE).  Next call
            // sees the encoded state and falls into reduce.
            if action <= YY_MAX_SHIFTREDUCE {
                let encoded = action + (YY_MIN_REDUCE - YY_MIN_SHIFTREDUCE);
                self.stack.push(Frame {
                    state: encoded,
                    major: major as u16,
                    value,
                });
                return Ok(false);
            }

            if action == YY_ACCEPT_ACTION {
                self.accepted = true;
                self.on_parse_accept();
                return Ok(true);
            }

            // Reduce.
            if action >= YY_MIN_REDUCE
                && action != YY_ERROR_ACTION
                && action != YY_NO_ACTION
            {
                let ruleno = (action - YY_MIN_REDUCE) as u32;
                match self.reduce(ruleno)? {
                    ReduceOutcome::Continue => continue,
                    ReduceOutcome::Accept => {
                        self.accepted = true;
                        return if token_code == 0 { Ok(true) } else { Ok(false) };
                    }
                }
            }

            // Try fallback once.
            if HAS_FALLBACK
                && !retried_with_fallback
                && major >= 0
                && (major as usize) < YY_FALLBACK.len()
                && YY_FALLBACK[major as usize] != 0
            {
                major = YY_FALLBACK[major as usize] as i32;
                retried_with_fallback = true;
                continue;
            }

            // No action.
            self.errored = true;
            return Err(ParseError::SyntaxError {
                token: token_code,
                state: cur_state,
            });
        }
    }

    /// Feed end-of-input.  Equivalent to push(0, 0).
    pub fn finalize(&mut self) -> Result<bool, ParseError> {
        self.push(0, Value::default())
    }

    /// Hook fired on syntax error.  Override the body via
    /// %rust_syntax_error in the grammar; default is no-op.
    pub fn on_syntax_error(&mut self, _token: u16, _state: u16) {
        /* no-op */
    }

    /// Hook fired on parse accept.  Override via
    /// %rust_parse_accept; default is no-op.
    pub fn on_parse_accept(&mut self) {
        /* no-op */
    }

    /// Hook fired on parse failure (post-error, no recovery).
    /// Override via %rust_parse_failure; default is no-op.
    pub fn on_parse_failure(&mut self) {
        /* no-op */
    }

    /// Hook fired on stack overflow.  In Rust the Vec stack
    /// grows automatically; this is mostly informational.
    /// Override via %rust_stack_overflow; default is no-op.
    pub fn on_stack_overflow(&mut self) {
        /* no-op */
    }

    #[inline(always)]
    fn top_state(&self) -> u16 {
        self.stack.last().map(|f| f.state).unwrap_or(0)
    }

    #[inline(always)]
    fn find_shift_action(state: u16, lookahead: u16) -> u16 {
        // state passthrough: states above YY_MAX_SHIFT mean
        // pending reduce; caller has already handled those.
        if state as u32 >= NSTATE {
            return state;
        }
        let ofst = YY_SHIFT_OFST[state as usize];
        let idx = ofst + (lookahead as i32);
        if idx < 0 || (idx as usize) >= YY_LOOKAHEAD.len() {
            return YY_DEFAULT[state as usize];
        }
        if YY_LOOKAHEAD[idx as usize] != lookahead {
            return YY_DEFAULT[state as usize];
        }
        YY_ACTION[idx as usize]
    }

    fn reduce(&mut self, ruleno: u32) -> Result<ReduceOutcome, ParseError> {
        if ruleno >= NRULE {
            self.errored = true;
            return Err(ParseError::BadReduce { rule: ruleno });
        }
        let nrhs = -(YY_RULE_NRHS[ruleno as usize] as i32);
        let lhs_sym = YY_RULE_LHS[ruleno as usize] as i32;

        // Snapshot RHS values, build a transient ReduceCtx,
        // call the callback, then mutate the stack.
        // Need at least nrhs frames above the bottom marker.
        if (self.stack.len() as i32) < nrhs + 1 {
            self.errored = true;
            return Err(ParseError::StackUnderflow);
        }
        let split = self.stack.len() - (nrhs as usize);
        let mut lhs_value = Value::default();
        // Reuse the per-parser scratch Vec to avoid an alloc
        // per reduce.  Hot path: amortises to zero.
        self.rhs_scratch.clear();
        self.rhs_scratch.extend(
            self.stack[split..].iter().map(|f| f.value.clone()));
        let cb = YY_RULE_REDUCE_FN[ruleno as usize];
        // SAFETY: split-borrow user from rhs_scratch via raw
        // pointers.  Both are owned by self; we're calling a
        // user fn that gets disjoint &mut refs to them and
        // self.stack isn't touched.  This is the same shape
        // as Vec::split_at_mut but across struct fields.
        let user_ptr: *mut UserArg = &mut self.user;
        cb(&mut ReduceCtx {
            lhs: &mut lhs_value,
            rhs: &mut self.rhs_scratch,
            user: unsafe { &mut *user_ptr },
        });

        // Pop nrhs frames.
        self.stack.truncate(split);

        // Compute goto state for the LHS non-terminal at the
        // parent state.  When the goto is YY_ACCEPT_ACTION,
        // the start rule has reduced to its bottom-of-stack
        // and we accept.  Mirrors src/parse_engine.c.
        let parent_state = self.top_state();
        let goto = Self::find_goto(parent_state, lhs_sym as u16);
        if goto == YY_ACCEPT_ACTION {
            self.final_value = lhs_value;
            self.on_parse_accept();
            return Ok(ReduceOutcome::Accept);
        }
        if goto == YY_ERROR_ACTION || goto == YY_NO_ACTION {
            self.errored = true;
            self.on_syntax_error(lhs_sym as u16, parent_state);
            return Err(ParseError::SyntaxError {
                token: lhs_sym as u16,
                state: parent_state,
            });
        }
        // Encode shift-reduce range as pending reduce on push.
        let pushed = if goto > YY_MAX_SHIFT && goto <= YY_MAX_SHIFTREDUCE {
            goto + (YY_MIN_REDUCE - YY_MIN_SHIFTREDUCE)
        } else {
            goto
        };
        self.stack.push(Frame {
            state: pushed,
            major: lhs_sym as u16,
            value: lhs_value,
        });
        Ok(ReduceOutcome::Continue)
    }

    #[inline(always)]
    fn find_goto(state: u16, lhs: u16) -> u16 {
        if (state as u32) >= NSTATE {
            return state;
        }
        let ofst = YY_REDUCE_OFST[state as usize];
        let idx = ofst + (lhs as i32);
        if idx < 0 || (idx as usize) >= YY_LOOKAHEAD.len() {
            return YY_DEFAULT[state as usize];
        }
        if YY_LOOKAHEAD[idx as usize] != lhs {
            return YY_DEFAULT[state as usize];
        }
        YY_ACTION[idx as usize]
    }
}

impl Default for JsonParseParser {
    fn default() -> Self { Self::new() }
}

enum ReduceOutcome { Continue, Accept }
