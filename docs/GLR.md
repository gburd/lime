...
**Experimental — feat/glr branch.**  Not enabled on `main`.

This document captures the resurrected GLR support from commit
`1506723^`, refactored to take a `ParserSnapshot *` and exposed
through `lime_parse_glr()`.  Includes measured performance vs LALR
on the same inputs; the overhead numbers are the basis for the
...
## Measured performance (m3pro, debugoptimized)
...
| GLR overhead vs LALR| +595% to +749% |

Variance between runs is significant on a developer laptop; the
shape of the result (~7–8× slowdown) is consistent.
