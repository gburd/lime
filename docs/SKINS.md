...
### Sharing tokens + YYSTYPE with a separate lexer (`%yystype_header`)

When the parser and the lexer are generated as **separate** units
-- `lime parser.lime` for the grammar and `lime -X lexer.lex` for
the scanner -- both sides need the same token kinds (`#define NUM 1`
...) and, if the lexer's action code sets semantic values, the same
`YYSTYPE`.  Historically this meant hand-writing a `_yytype.h`
bridge.  Lime now owns it:

```
%yystype_header "calc_types.h"     /* in the grammar, OR ... */
```
```sh
lime --type-header calc.lime        # ... emits calc_yytype.h
```

The emitted header is plain and self-contained (no makeheaders/`-m`
toolchain required):

```c
/* Generated by lime --type-header.  DO NOT EDIT. */
#ifndef LIME_CALC_TYPES_H
#define LIME_CALC_TYPES_H

#define NUM                                1
#define PLUS                               2

#ifndef YYSTYPE_IS_DECLARED
typedef union { /* %union body */ } YYSTYPE;
#define YYSTYPE_IS_DECLARED 1
#define YYSTYPE_IS_TRIVIAL 1
#endif

#endif /* LIME_CALC_TYPES_H */
```

The lexer `.c` (or any hand-written scanner) simply `#include`s it
and returns the `#define`d kinds / fills a `YYSTYPE`.  Because the
typedef is guarded by `YYSTYPE_IS_DECLARED`, it composes with the
parser `.c` (whose own YYSTYPE typedef carries the same guard) when
both land in one translation unit -- no redefinition.

The value type follows the same precedence as everywhere else:
`%union` first, else `%token_type`.  When neither is set, only the
token `#define`s are emitted (the lexer does not need the parser's
internal stack type).

- `%yystype_header "NAME"` -- directive form; chooses the basename.
- `--type-header` -- flag form; uses the default `<stem>_yytype.h`.
- The directive takes precedence when both are present.

This is independent of the makeheaders (`-m`) path, which remains
available for projects already using that tool.
