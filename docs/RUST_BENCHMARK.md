...
## Multi-shape benchmark validation (v0.8.9)

The v0.8.4 / v0.8.9 benchmarks above use a single 226 KB JSON
fixture.  That fixture is too small to exercise the SIMD fast
path meaningfully (average string body is 9 bytes, fits in one
AVX2 chunk).  v0.8.9 looked like "no win" on that fixture
specifically, which is misleading.

Running against larger, schema-driven fixtures generated by
`bench/rust_compare/make_fixture_v2.py` reveals the real impact.
Five document shapes, each 1MB and 5MB:

  api_log    HTTP API access logs (mixed structural + short strings)
  product    e-commerce catalogue (medium descriptions, nested specs)
  tweet      social posts (short text, hashtags, mentions)
  metric     telemetry time-series (heavy on numbers, sparse strings)
  prose      documents with long natural-language bodies (1k-10k char strings)

### 1MB fixture, tokenize MB/s, best of 3

| shape | lime | lime+simd | lime+memchr | logos | lalrpop | v0.8.9 vs scalar |
|---|---:|---:|---:|---:|---:|---|
| **prose**   | 348.2 | **685.5** | 644.5 | 432.3 | 459.1 | **+97% (1.97×)** |
| product | 184.6 | **228.7** | 211.8 | 289.2 | 310.5 | +24% |
| tweet   | 140.1 | **155.3** | 142.5 | 254.1 | 270.9 | +11% |
| api_log | 146.5 | **154.9** | 142.7 | 267.3 | 286.0 |  +6% |
| metric  |  96.4 |  93.4 |  87.6 | 345.5 | 347.7 |  -3% |

### 5MB fixture, tokenize MB/s, best of 5

| shape | lime | lime+simd | lime+memchr | logos | lalrpop | v0.8.9 vs scalar |
|---|---:|---:|---:|---:|---:|---|
| **prose**   |  67.4 | **130.7** | 133.0 |  88.7 |  90.5 | **+94% (1.94×)** |
| product |  35.1 |  42.5 |  38.8 |  53.7 |  57.1 | +21% |
| api_log |  15.0 |  15.7 |  14.8 |  48.6 |  51.4 |  +5% |
| tweet   |  15.6 |  16.2 |  15.4 |  47.1 |  47.7 |  +4% |
| metric  |  10.4 |  10.0 |   9.7 |  63.4 |  63.9 |  -4% |

### Headline finding

**On prose-heavy content, `lime --rustlex --rustlex-simd` BEATS logos
by 1.46×–1.59×** at both 1MB and 5MB scales.  This is the architecture
working as designed: long string-body runs (1k-10k chars per body) let
AVX2's 32-byte chunks fire 30+ times per token instead of once.

On JSON-shape content (api_log / tweet / metric — many short tokens),
lime is still 3-6× behind logos.  This is **not** a SIMD scan-dispatch
issue — it is the per-token DFA architectural difference plus the
allocation cost of returning `Vec<Token>` (logos uses a streaming
iterator that doesn't allocate).  Two separable improvements would
close most of that gap:

1. **Streaming iterator API** for lime's Rust output (so consumers
   avoid the Vec<Token> allocation when they don't need a buffered
   token list).  ~50 LOC.

2. **Per-token DFA emission** (~600-1000 LOC of new emit code at the
   shared DFA layer; benefits both Rust and C output).
...
for shape in api_log product tweet metric prose; do
    python3 make_fixture_v2.py --shape $shape --size-mb 1 \
        --out fixture_${shape}_1mb.json
done

# Benchmark
cargo build --release --bins
for shape in prose product api_log tweet metric; do
    fix="fixture_${shape}_1mb.json"
    for label in lime lime_simd lime_memchr logos lalrpop; do
        FIXTURE="$fix" ./target/release/bench_$label
    done
done
```

The fixture generator supports five shapes, sizes from kilobytes
to multi-GB, multi-document mode, NDJSON output, and reproducible
seeds.  See `make_fixture_v2.py --help`.
