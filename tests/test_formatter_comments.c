...
int main(int argc, char **argv) {
    ...
    /* Working copies in a per-process tmpdir so we don't litter
    ** the source tree and don't collide between concurrent runs. */
    char tmpdir[256];
    if (test_compat_tmpdir("lime_fmtcom", tmpdir, sizeof(tmpdir)) != 0) {
        fprintf(stderr, "FAIL: could not create tmpdir\n");
        return 1;
    }
    char work[512], fmt1[512], empty[512];
    snprintf(work, sizeof(work), "%s/input.lime", tmpdir);
    snprintf(fmt1, sizeof(fmt1), "%s/input.lime.formatted", tmpdir);
    snprintf(empty, sizeof(empty), "%s/empty.lime", tmpdir);
